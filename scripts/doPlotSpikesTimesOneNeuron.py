...
def main(argv):
    ...
    parser.add_argument("--session_id", help="session id", type=int,
                        default=256511)
    parser.add_argument("--neuron_to_plot", help="neurontrial to plot",
                        type=int, default=23453)
    ...
    spikes_times_torch = [[torch.from_numpy(spikes_times[r][n])
                           for n in range(n_neurons)]
                          for r in range(n_trials)]
    trials_indices = torch.arange(n_trials)
    cell_ids = loadRes["cell_ids"]
    subject_choice = loadRes["subject_choice"]
    ...
    fig = svGPFA.plot.plotUtilsPlotly.getSpikesTimesPlotOneNeuron(
        ...
        trials_indices=trials_indices, trials_colors=trials_colors,
        title=title)
    fig.add_vline(x=0, line_width=1, line_dash="dot", line_color="black")
    # fig.update_xaxes(range=xlim)
    spikes_times_png_filename = spikes_times_fig_filename_pattern.format(
        session_id, neuron_to_plot, "png")
    spikes_times_html_filename = spikes_times_fig_filename_pattern.format(
        session_id, neuron_to_plot, "html")
